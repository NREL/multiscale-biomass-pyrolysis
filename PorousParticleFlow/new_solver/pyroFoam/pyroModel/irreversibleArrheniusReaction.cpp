...
irreversibleArrheniusReaction::irreversibleArrheniusReaction(const dictionary& dict, const List<word>& species_name)
:
    m_name(dict.dictName()),
    ...
    m_product_index(m_product_stoch.size(), -1)
{
    ...
    forAll(species_name, specieI)
    {
        ...
        forAll(product_list, prodI)
        {
            ...
            {
                m_product_index[reactI] = specieI;
            }
        }
    }
    ...
    forAll(m_reactant_index, id)
    {
        ...
        {
            Info << "Error: unknown reactant " << reactant_list[id] << "\n" << exit(1);
        } 
    }
    ...
    forAll(m_product_index, id)
    {
        ...
        {
            Info << "Error: unknown product " << product_list[id] << "\n" << exit(1);
        } 
    }
